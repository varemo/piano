...
writeFilesForKiwi <- function(gsaRes, label="", overwrite=FALSE) {
   ...
   if(!is(gsaRes, "GSAres")) stop("argument gsaRes should be of class GSAres or a list of GSAres objects")
   if(!is(label, "character") | length(label)!=1) stop("argument label should be a character string")
   if(nchar(label)>0) {
